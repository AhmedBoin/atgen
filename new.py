...
class NeuroEvolution(ATGEN):
    def __init__(self, population_size: int, model: nn.Sequential):
        config = ATGENConfig(crossover_rate=0.6, mutation_rate=0.8, perturbation_rate=0.9, mutation_decay=0.95, perturbation_decay=0.95, direct_crossover=False)
    ...
    def fitness_fn(self, model: nn.Sequential):
        ...
        for _ in range(epochs):
            ...
            while True:
                ...
                total_reward += reward#(reward*(1-np.tanh(np.abs(action[0])/5)))
    ...
    def pre_generation(self):
        ...
        for epoch in range(50):
            ...
            loss.backward()
            self.optimizer.step()
            print(f"Epoch {epoch+1}, Loss: {loss.item()}, steps: {self.steps}", end="\r")
        print()
        self.lr_decay.step()
...
if __name__ == "__main__":
    ...
    # ne.load_population()
    ne.continual_evolution(fitness=1000)
