...
class NeuroEvolution(ATGEN):
    def __init__(self, population_size: int, model: nn.Sequential):
        ...
        memory = ReplayBuffer(buffer_size=4, steps=70, dilation=20, similarity_cohort=10, accumulative_reward=True)
    ...
    def fitness_fn(self, model: nn.Sequential):
        ...
        while True:
            ...
            if terminated or truncated:
                break
        env.close()
        return (total_reward - self.memory.lower_bound) / steps
    ...
    def experiences_fn(self, model: nn.Sequential):
        ...
        for _ in range(epochs):
            ...
            while True:
                ...
                total_reward += reward
                self.memory.track(state, action, (total_reward - self.memory.lower_bound) / steps)
                state = next_state
                
                if terminated or truncated:
                    break
        env.close()
        print(f"Total reward: {(total_reward - self.memory.lower_bound) / steps}")
